...
Небольшой глоссарий. Под библиотекой подразумевается код и прочие артефакты сторонних разработчиков. Под пакетом подразумевается конкретный способ сборки и упаковки библиотеки.
...
Версия пакетаформируется по шадобну *версия_библиотеки*+*номер_версии_пакета*. Отсчет номера версии пакета начинается с нуля. При изменении рецепта и\или экспортируемых файлов необходимо сдвигать версию пакета вперед.

### Система контроля версий и дерево исходников пакета

**master** - в этой ветке находится последний стабильный релиз (версия пакета, используемая в качестве готового компонента в других проектах)

**develop** - ветка для разработки. В ней могут меняться как исходники (версия) самой библиотеки, так и способ опакечивания библиотеки. После завершения изменений содержимое ветки переносится в **master** (выпуск релиза).

**release/a.b.c** - где *a.b.c* - версия библиотеки. Последняя вариация пакета для конкретной версии библиотеки. Сюда бекпортируются изменения из ветки *develop*
...
Включаются опцией *with_unit_tests*. Если она установленна нужно передать системе сборки библиотеки соответствующую опцию для сборки и запуска тестов. (для CMake см. пример сборки)


## Упаковка продуктов

Большинство библиотек стоит упаковывать запуском инсталяции системы сборки. После инсталяции рекомендуется удалить лишние файлы из пакета (см. пример сборки). Не забываем упаковывать скрипт интеграции с **CMake** (если он есть, см. раздел *Интеграция с CMake*).
...
Если у библиотеки есть публичный скрипт интеграции с CMake (библиотеку можно подключть к проекту на CMake командой *find_package*), то нужно написать аналогичный скрипт повторяющий функциональность. Без него намного сложней подключать Conan-библиотеки к сторонним проектам (приходится накладывать патчи, они сосуществовании CONAN_PKG::library они не в курсе).

Тут пример такого скрипта (OpenSSL)

Если публичного скрипта изначально нет, то этот момент можно опусть и просто использовать в своих проектах CONAN_PKG::library. Пример - наша библиотека *jscript*
